...
docker run --name CluedIn.Documentation --rm -e "JEKYLL_ENV=docker" --volume="${PWD}:/srv/jekyll" --publish 4000:4000 jekyll/jekyll bash -c "gem install bundler:2.2.27 && jekyll serve --verbose --livereload --config _config.yml,_config.docker.yml"