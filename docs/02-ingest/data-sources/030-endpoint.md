...
grand_parent: Ingest
nav_order: 30
permalink: /ingest/data-sources/endpoint
content_type: how-to
redirect_from: ["/integration/endpoint"]
source_path: docs/040-integration/data-sources/150-endpoint.md
tags: ["integration"]
...
{:.important}
The maximum number of columns accepted for data ingestion is 490.<br>Before loading data into CluedIn, ensure that it does not contain unsupported characters. Refer to our knowledge base [article](/model/data-catalog/supported-characters) for information about supported and unsupported characters.
...
(For advanced users) To ensure the creation of the expected golden records, you can generate a sample clue and verify its accuracy before processing. This step helps confirm that the resulting golden record aligns with your expectations. For more information, see [Clue](/get-started/core-concepts/clue-reference).
...
1. In Postman, paste the URL that you copied to the URL input field of your request. Then, send the request.
    ...
    The data has been sent to CluedIn. You can now view it on the [Preview](/ingest/processing/preview) tab of the data set. The next steps involve [creating a mapping](/ingest/mapping/create-mapping) and [processing the data](/ingest/processing/process-data).
...
1. Every time new data arrives through the endpoint, CluedIn checks whether it matches the expected mapping:
    ...
    - If the data contains new fields (outside of the current mapping), these fields appear on the **Map** tab. They are marked with a **Warning** label and remain unmapped.
        ...
        You can choose to [ignore](/ingest/mapping/create-mapping#ignore-fields-for-an-ingestion-endpoint) these new fields in the [mapping](/ingest/mapping/create-mapping). When ignored, the fields are skipped during processing and do not appear in the resulting golden records. Ignoring fields is useful to prevent records with new, unmapped fields from being quarantined.

       ![ignore_field_btn.png]({{ "/assets/images/integration/data-sources/endpoint/ignore_field_btn.png" | relative_url }})

1. When you [process the data](/ingest/processing/process-data), the following takes place:

    - Records that contain new, unmapped fields (that were not ignored) are moved to [quarantine](/ingest/troubleshooting/quarantine).
...
- A mapping was [created](/ingest/mapping/create-mapping). When schema protection is enabled, CluedIn will catch all incoming fields outside of this mapping.
...
1. Confirm that you want to enable schema protection.
    ...
    Now, when the endpoint receives fields that are outside of the defined mapping, these fields will appear on the **Map** tab, marked with a **Warning** label. If needed, proceed to [ignore](/ingest/mapping/create-mapping#ignore-fields-for-an-ingestion-endpoint) these fields.
