...
grand_parent: Ingest
nav_order: 30
permalink: /ingest/processing/validations
content_type: how-to
redirect_from: ["/integration/additional-operations-on-records/validations"]
source_path: docs/040-integration/additional-operations-on-records/090-validations.md
...
- The data set should be **mapped** to standard fields. This way CluedIn can analyze your data and suggest appropriate validation methods. Learn more in [Create mapping](/ingest/mapping/create-mapping) and [Review mapping](/ingest/mapping/review-mapping).

- The data set should be in the **edit mode**. This way you can add validations and modify source records. Learn more in [Preview](/ingest/processing/preview).
...
After running the validation checks, you can start reviewing and fixing invalid values. You are not required to fix invalid values—when you process the data set, the records containing invalid values will be processed in the same way as all the other records and they will not be automatically sent to the [quarantine](/ingest/troubleshooting/quarantine) or for approval area unless there are specific rules.
