...
grand_parent: Ingest
nav_order: 40
permalink: /ingest/mapping/preprocess-rules
content_type: how-to
redirect_from: ["/integration/additional-operations-on-records/preprocess-rules"]
source_path: docs/040-integration/additional-operations-on-records/020-preprocess-rules.md
tags: ["integration", "pre-process rules"]
last_modified: 2024-01-15
---

Pre-process rules are applied to the records that are already mapped ([clues](/get-started/core-concepts/clue-reference)). You can create pre-process rules when you want to add tags or aliases to the records or send the records to quarantine. The difference between [property rules](/ingest/mapping/property-rules) and pre-process rules is that property rules are applied only to property values of the record while pre-process rules are applied to the whole record. So, if you want to apply some changes to multiple records, create a pre-process rule.
