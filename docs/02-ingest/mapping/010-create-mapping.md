...
grand_parent: Ingest
nav_order: 10
permalink: /ingest/mapping/create-mapping
content_type: how-to
redirect_from: ["/integration/create-mapping"]
source_path: docs/040-integration/data-sources/170-create-mapping.md
...
1. In the lower-right corner, select **Add Property to Annotation**.
    ...
    The mapping is created. Next, [review the mapping details](/ingest/mapping/review-mapping).
...
{:.important}
To use AI capabilities to create mapping, first complete all the steps described in [Azure Open AI Integration](/solutions/open-ai-integration).
...
By default, all fields included in the mapping are [processed](/ingest/processing/process-data) and appear in the resulting golden records. If you do not want a field to be processed, you can choose to ignore it.

### Ignore fields for an ingestion endpoint

When data is [ingested through an endpoint](/ingest/data-sources/endpoint) with [schema protection](/ingest/data-sources/endpoint#schema-protection) enabled, CluedIn checks whether the incoming data matches the mapping defined for that endpoint:

- If the data contains new, unmapped fields (outside of the defined mapping), the records are moved to [quarantine](/ingest/troubleshooting/quarantine) during [processing](/ingest/processing/process-data).
...
- Depending on whether you have already processed the data set, do one of the following:
    ...
    - If you have already processed the data set:
        ...
        1. Remove records created from the data set following the instruction [here](/ingest/processing/remove-records).

        1. Remove processing logs of the data set following the instruction [here](/ingest/processing/process-data#remove-processing-logs).
