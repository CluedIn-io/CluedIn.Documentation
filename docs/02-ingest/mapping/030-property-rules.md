...
content_type: how-to
redirect_from: ["/integration/additional-operations-on-records/property-rules"]
source_path: docs/040-integration/additional-operations-on-records/010-property-rules.md
...
Property rules help you improve the quality of mapped records ([clues](/get-started/core-concepts/clue-reference)) by normalizing and transforming property values. The difference between property rules and [pre-process rules](/ingest/mapping/preprocess-rules) is that property rules are applied only to property values of the record while pre-process rules are applied to the whole record. So, if you want to apply some changes to the property values, create a property rule.
...
- Quarantine – send records to the [quarantine](/ingest/troubleshooting/quarantine) where you can modify them and decide what to do with them afterwards.
...
{:.important}
To use AI capabilities to create property rules, first complete all the steps described in [Azure Open AI Integration](/solutions/open-ai-integration).
