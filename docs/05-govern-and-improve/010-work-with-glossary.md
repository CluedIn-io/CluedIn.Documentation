...
parent: Govern & improve
nav_order: 10
permalink: /govern/work-with-glossary
content_type: how-to
redirect_from: ["/getting-started/glossary"]
source_path: docs/010-getting-started/080-glossary.md
...
**Before you start:** Make sure you have completed all steps in the [Ingest data guide](/ingest/tutorial-ingest-data) and [Stream data guide](/publish/streams/tutorial-stream-data).
...
1. Go to the **Matches** tab to view the records that meet the condition that you set up.
    ...
    By default, all records are displayed in the following columns: **Name**, **Business Domain**, and **Description**. To add more columns to the table, see step 3 of [Find data](/govern/cleaning/tutorial-clean-data#find-data).
...
Now, you can clean the glossary term and stream it to a Microsoft SQL Server database. As an example, [update the configuration of the stream](#update-stream-configuration) that you created in the [Stream data](/publish/streams/tutorial-stream-data) guide.
...
Next, learn how to add edges to build relations between golden records in the [Add relations between records](/model/relationships/tutorial-add-relations).