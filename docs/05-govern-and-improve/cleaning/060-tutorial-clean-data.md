...
nav_order: 60
permalink: /govern/cleaning/tutorial-clean-data
content_type: tutorial
redirect_from: ["/getting-started/manual-data-cleaning"]
source_path: docs/010-getting-started/030-manual-data-cleaning.md
...
**Before you start:** Make sure you have completed all steps in the [Ingest data guide](/ingest/tutorial-ingest-data).
...
**Useful links:** [Search](/master/search), [Filters](/master/filters)
...
1. Select **Search**. The records that match the filter criteria are displayed on the search results page. After finding the records, [save the search](/master/search#saved-searches). This way you can quickly verify if the values have been cleaned.
...
1. In the upper-right corner, select **Process**. In the confirmation dialog box, select **Skip** and leave the **Enable automatic generation of data part rules** checkbox cleared. Then, confirm that you want to process the data.
    ...
    CluedIn automatically identifies the changes and updates the records. To verify that your changes have been applied, [retrieve the saved search](/master/search#saved-searches).

{:.important}
All changes to the records in CluedIn are tracked. You can search for the needed record and on the **History** pane, you can view all actions associated with the record. For more information, see [History](/master/golden-records/history).
...
The next item on the list of common data management tasks is deduplication. Learn how to identify and merge duplicates in the [Deduplicate data guide](/master/deduplication/tutorial-deduplicate-data).