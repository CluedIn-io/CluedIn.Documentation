...
nav_order: 10
permalink: /govern/cleaning/create-clean-project
content_type: how-to
redirect_from: ["/preparation/clean/create-clean-project"]
source_path: docs/050-preparation/clean/010-create-clean-project.md
...
- [From the Tag Monitoring module](/govern/tags/tag-monitoring) – this option is useful for creating a clean project that includes all records flagged with a specific tag.
...
1. In the search field, select the search icon. Depending on whether you have the saved search that retrieves the data you need to clean, do one of the following:
    ...
    - If you have the saved search, in the upper-right corner of the page, select the vertical ellipsis button, and then select **Saved Searches**. Find and open the needed saved search.
        ...
        Make sure that the vocabulary keys to be cleaned are displayed on the search results page. This way, you don't have to specify the vocabulary keys in the clean project configuration. To learn how to add vocabulary keys to the search results page, see [Add columns](/master/search#add-columns).

    - If you don't have the saved search, define the data that you want to clean using [filters](/master/filters). Then, add the vocabulary keys to be cleaned to the search results page following the [Add columns](/master/search#add-columns) procedure. If you need to clean the same set of data in future, save the current filter configuration.
...
1. In the lower-right corner, select **Create**.
    ...
    You created the clean project from the search results page. The next step is to [generate results](/govern/cleaning/manage-clean-project#generate-results) of the clean project.
...
1. In the lower-right corner, select **Create**.
    ...
    You created the clean project from the Preparation module. The next step is to [generate results](/govern/cleaning/manage-clean-project#generate-results) of the clean project.
...
1. In the lower-right corner, select **Create**.
    ...
    You created the clean project from the glossary. The next step is to [generate results](/govern/cleaning/manage-clean-project#generate-results) of the clean project.