...
nav_order: 10
permalink: /govern/tags/tag-monitoring
content_type: concept
redirect_from: ["/governance/tag-monitoring"]
source_path: docs/070-governance/010-tag-monitoring.md
...
1. By default, the **Tag Monitoring** page shows the data for all [business domains](/model/business-domains/business-domains-explained) and for the last month going back from today.
...
1. On the tags panel on the left, select the tag of interest.
    ...
    1. The [rule](/govern/rules) that was used to apply this tag.

    1. [Rule type](/govern/rules/rule-types) (data part rule or golden record rule).
...
1. At the bottom of the page, review the list of golden records tagged with the selected tag.
    ...
    - Select which properties should appear in the table. To do this, above the table, select **Column Options**. Then, select the needed properties or vocabulary keys. For details on how to work with column options, see [Search](/master/search#search-results-page).
...
You can leverage [AI agents](/govern/ai-agents) to fix the golden records flagged with a specific tag.

**Prerequisites**

- The AI agents feature must be enabled. For details, see [Prerequisites to using AI agents](/govern/ai-agents/prerequisites-to-using-ai-agents).
...
1. In the upper-right corner of the page, select **Fix Data Quality Issues with AI**.
    ...
    {:.important}
    If the **Fix Data Quality Issues with AI** button is unavailable, this may mean that the AI agents feature is not [enabled](/govern/ai-agents/prerequisites-to-using-ai-agents).
...
1. On the **Fix Data Quality Issues with AI** panel, do the following:
    ...
    An [AI agent job](/govern/ai-agents/create-configure-and-run-an-ai-agent#create-a-job) is created to handle the selected tagged records. A corresponding label appears in the upper-right corner of the page.
...
1. Configure, test, and run the AI agent job:
    ...
    1. Go to the **Configuration** tab of the job. Note that the **Data** and **Skills** sections are filled out for you.
      ...
      1. [Test the job](/govern/ai-agents/create-configure-and-run-an-ai-agent#test-the-job) to verify that it fixes the records as expected.

         ![test_job_btn.png]({{ "/assets/images/governance/tag-monitoring/fix-records-with-ai/Images/test_job_btn.png" | relative_url }})

      1. [Run the job](/govern/ai-agents/create-configure-and-run-an-ai-agent#run-the-job) and [review its results](/govern/ai-agents/review-the-results-returned-by-an-ai-agent) to have the records fixed.
...
After you [use](#monitor-tag-usage) the **Tag Monitoring** page to display the list of golden records flagged with a specific tag, you can create a [clean project](/govern/cleaning) for these golden records.
...
1. On the **Choose Vocabulary Keys** tab, do the following:
    ...
    You created the clean project from the Tag Monitoring module. The next step is to [generate results](/govern/cleaning/manage-clean-project#generate-results) of the clean project.
...
{:.important}
You can also use the [Rule Builder module](/govern/rules) to create new tags. For a step-by-step walkthrough, see [Identifying and labelling incorrect data](/learning-paths/fundamentals/identifying-and-labelling-incorrect-data).
...
1. Select the **Type** of rule that will be created to apply the tag – data part rule or golden record rule. For details about these rules types, see:
    ...
    - [Data part rules](/govern/rules/rule-types#data-parts-rules)

    - [Golden record rules](/govern/rules/rule-types#golden-record-rules)
