...
nav_order: 70
permalink: /govern/rules/tutorial-create-rules
content_type: tutorial
redirect_from: ["/getting-started/rule-builder"]
source_path: docs/010-getting-started/060-rule-builder.md
...
1. Choose the [type of rule](/govern/rules/rule-types) that you want to create. Then, select **Create Rule**.
...
1. In the **Filters** section, select **Add First Filter**, and then specify to which items the rule will be applied:
    ...
    1. Select the value of the property or vocabulary key.
        ...
        The fields for configuring a filter appear one by one. After you complete the previous field, the next field appears. For more information, see [Filters](/master/filters).
...
1. In the **Actions** section, select **Add Action**, and then configure the action that CluedIn can perform on the filtered items:
    ...
    1. In the **Action** section, select the action to be performed by the rule. To learn more about what each action does, see [Rules reference](/govern/rules/rules-reference).
...
1. Depending on whether the rule applies to the processed or unprocessed data, do one of the following:
    ...
    - If the rule applies to the unprocessed data, process the data as described in the [Ingest data guide](/ingest/tutorial-ingest-data). In this case, the rule will be applied to the records automatically during processing.
...
Next, learn how to visualize relations between golden records with the help of Hierarchy Builder in our [Create hierarchies guide](/model/hierarchies/tutorial-create-hierarchies).
