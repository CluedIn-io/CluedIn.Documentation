...
grand_parent: Model
nav_order: 30
permalink: /model/data-catalog/data-types
content_type: reference
redirect_from: ["/management/data-catalog/data-types"]
source_path: docs/080-management/data-catalog/050-data-types.md
---
...
| DateTime | A date with a time, in the time zone of your computer. If no time zone is specified, then UTC is assumed when we try to normalize the date.<br>Date normalization occurs when the **Date Time** option is enabled in **Administration** > **Settings** > **Processing Property Data Type Normalization**. In this case, CluedIn analyzes the incoming date format and converts it to ISO 8601 format (YYYY-MM-DDT00:00:00+00:00). If you want to instruct CluedIn how to interpret dates before converting them to ISO 8601 format, [create](/govern/rules/create-rule) a rule with the **Normalize Date** [action](/govern/rules/rules-reference). | Typed |
...
| Boolean | A _true_ or _false_ value. Data will only be strong-typed if it is stored as the values "true" or "false". For all other values such as "yes", "no", "1", "0" and so on, the data must be normalized using a [rule](/govern/rules) or a clean [project](/govern/cleaning). | Typed |
...
| [Lookup](/model/data-catalog/lookup-data-type) | A custom list of possible values, which is defined with a glossary. | Keyword |
