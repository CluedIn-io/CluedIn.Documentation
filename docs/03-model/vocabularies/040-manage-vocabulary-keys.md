...
grand_parent: Model
nav_order: 40
permalink: /model/vocabularies/manage-vocabulary-keys
content_type: how-to
redirect_from: ["/management/data-catalog/manage-vocabulary-keys"]
source_path: docs/080-management/data-catalog/040-manage-vocabulary-keys.md
---
...
Depending on the selected [data modeling approach](/model/data-catalog/modeling-approaches), you can create a vocabulary key in two ways:

- **Automatically** – this option is part of the data-first approach. When [creating a mapping](/ingest/mapping/create-mapping) for a data set, you have the option to generate a new vocabulary. CluedIn will then automatically create vocabulary keys and assign data types. You can customize the names of vocabulary keys and data types. Once the mapping is created, you can then open the vocabulary and make additional adjustments to vocabulary keys if needed.
...
1. Select the data type of the vocabulary key. For more information, see [Data types](/model/data-catalog/data-types).
...
The purpose of vocabulary key mapping is to keep data organized and consistent by preventing duplicate values in the system. For the practical application of vocabulary key mapping, check the video in [Modeling approaches](/model/data-catalog/modeling-approaches).
