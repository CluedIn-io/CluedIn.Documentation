...
The installer will attempt to detect the permisisons of the user installing the application. If they do not match the required permission set then it will fail validation.
...
The difference license type have diffent hourly rate and other restrictions so please read the license information carefully.
...
The organization name will be used as part of the URL so will ideally be one word with no dashes or hyphens.
...
Most of the time there will be nothing to adjust on this page unless advised to by CluedIn Support.
...
The processing node-pool can have auto-scaling enabled, meaning that as CPU pressure builds (high work load) in this node pool it will allow AKS to start up new nodes to compensate for the increase in load.

The extra nodes used will be shut down when load returns to normal levels again but there is an extra infrastructure and possible licensing cost incured by scaling up.

Please contact CluedIn support for more information.
...
There are different options to let the installer use existing or provision new services as part of the setup. Many of these options can incur extra infrastructure charges from Azure as new resources are provisioned or used.
...
The installer can provision an Azure SQL (Elastic Pool) to install all the databases on. The Azure SQL resource will be created inside the managed resource group.
...
The installer can also provision a new Azure Redis Cache for use by CluedIn. The Azure Redis Cache resource will be created inside the managed resource group.
...
This tab contains technical and debugging switches for use by CluedIn Support. The majority of these options should not be touched and can cause the installation to fail if mis-configured or changed. 

Only make modifications on the advice of CluedIn Support personnel. 

#### Advanced Networking

There may be a requirement to change the address CIDR ranges of the VNET that gets created. For example, if connecting to a VNET that has similar internal IP addresses, and you wish to avoid a clash.
...
The installer is unable to verify these IP ranges and their validity before the install starts, so please double-check all settings before proceeding.
