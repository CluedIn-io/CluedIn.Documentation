---
layout: cluedin
title: Plan the processing
parent: Data ingestion playbook
grand_parent: Playbooks
nav_order: 50
permalink: /playbooks/data-ingestion/plan-the-processing
content_type: concept
redirect_from: ["/playbooks/data-ingestion-playbook/process-data"]
source_path: docs/210-playbooks/data-ingestion/005-process-data.md
---
...
- Make sure you have [created the mapping](/playbooks/data-ingestion/plan-the-mapping) for your records.
...
A billable record is a **new record** that was processed from a data source. Learn more about billable records in a dedicated [article](/get-started/core-concepts/billable-records).
...
If you do not want to send specific records from the data set to the processing pipeline, you can set up rules to send such records to quarantine. Learn more about quarantine in a dedicated [article](/ingest/troubleshooting/quarantine).

**Modify records before processing**

- **Advanced mapping** – you can write a glue code in your data set to modify the records before they are processed. This is useful for complex and conditional mapping. Learn more about advanced mapping code [here](/ingest/mapping/advanced-mapping-code).

- **Property rules** – improve the quality of records by normalizing, transforming, or changing property values before the records are processed. Learn more about property rules [here](/ingest/mapping/property-rules).

- **Pre-process rules** – execute some basic operations on the records before they are processed. Learn more about pre-process rules [here](/ingest/mapping/preprocess-rules).
...
You can find step-by-step instruction for processing records in a dedicated [article](/ingest/processing/process-data). Once the processing is done, you will see the result similar to the following.

![process-records-result.png]({{ "/assets/images/playbooks/process-records-result.png" | relative_url }})

## Next steps

Now that your records have turned to golden records, you can start searching for them using the main [search](/master/search).
