...
nav_order: 30
permalink: /publish/streams/manage-streams
content_type: how-to
redirect_from: ["/consume/streams/manage-streams"]
source_path: docs/150-consume/streams/020-manage-streams.md
...
You can edit the stream configuration and the export target configuration regardless of the [stream status](/publish/streams/stream-reference#stream-statuses).
...
1. Select **Duplicate**.
    ...
    The new stream is created. By default, the export target for the stream is not configured. Now, you can modify the stream configuration if needed and [configure](/publish/streams/create-a-stream#configure-an-export-target) the export target. When you reach the desired configuration, [start](/publish/streams/manage-streams#start-pause-and-stop-a-stream) the stream.
