...
parent: Master Data Services
grand_parent: Solutions & integrations
nav_order: 10
permalink: /solutions/master-data-services/direct-connection
content_type: how-to
redirect_from: ["/microsoft-integration/mds-integration/direct-connection"]
source_path: docs/180-microsoft-integration/mds/010-direct-connection.md
---
...
In this article, you will learn how to connect CluedIn to on-premises Master Data Services (MDS) directly over HTTPS. MDS only accepts Windows (NTLM/Negotiate) authentication by default, which is not supported from CluedIn's Linux-based crawler containers, so a dedicated Basic authentication endpoint is exposed instead. Use this approach if CluedIn has a routable network path to your on-premises MDS server - if it doesn't, see [Azure Relay](/solutions/master-data-services/azure-relay) instead.
