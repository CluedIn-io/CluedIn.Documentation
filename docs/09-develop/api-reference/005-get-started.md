...
content_type: how-to
redirect_from: ["/rest-api/get-started"]
source_path: docs/250-rest-api/010-get-started.md
tags: ["api", "rest api"]
---
...
This page explains how to connect to the CluedIn REST API: base URLs, authentication, and the conventions that requests and responses follow. For the full list of endpoints, see the [API reference](/api).

## How the API is organized

The API has more than 700 endpoints, grouped into [categories](/api). Rather than listing them alphabetically, the tree below organizes those categories around the **problem each one solves**, following the path data takes through CluedIn—from modeling and mastering it to delivering it back out, plus the categories that operate, secure, and automate the platform around that flow.

Use it to find the right starting point: identify the question you are trying to answer, then open the matching category in the [API reference](/api).
...
|---|---|---|---|
| Operate the platform | [Administration & configuration](/api/administration-and-configuration) | How do I manage settings, logs, and metered billing? | 8 |
| Operate the platform | [Organization](/api/organization) | How do I manage the organization profile and usage statistics? | 31 |
| Model the data | [Vocabularies](/api/vocabularies) | How is my data typed and described with vocabulary keys? | 110 |
| Model the data | [Glossary](/api/glossary) | What do our shared business terms mean? | 57 |
| Model the data | [Hierarchies](/api/hierarchies) | How is data organized into hierarchies and the global data model? | 30 |
| Build golden records | [Entities](/api/entities) | What are my golden records—how do I read, modify, and merge them? | 115 |
| Build golden records | [Deduplication](/api/deduplication) | How do I find, review, and resolve duplicate records? | 61 |
| Build golden records | [Rules & evaluation](/api/rules-and-evaluation) | How are records transformed, merged, and survived; what did a rule do? | 66 |
| Build golden records | [Data preparation & enrichment](/api/data-preparation-and-enrichment) | How do I clean and enrich records? | 40 |
| Find & explore | [Search](/api/search) | How do I find records across the graph and manage saved searches? | 49 |
| Deliver data out | [Streams, connectors & export](/api/streams-and-export) | How do I push trusted data to downstream systems? | 62 |
| Govern & secure | [Access control & governance](/api/access-control-and-governance) | Who can do what; how do I handle ownership, audit, and tags? | 33 |
| Automate & augment | [AI](/api/ai) | How do I use AI agents, jobs, skills, and Copilot? | 59 |
...
{:.important}
For stability, prefer the explicit versioned path (`/api/v1/...`). Each endpoint's exact path and available versions are shown in the [API reference](/api).
...
You generate API tokens in the CluedIn UI under **Administration** > **API Tokens**. For step-by-step instructions, see [Generate an API token](/administer/api-token).
...
Endpoints that return collections commonly support the following query parameters for paging and sorting. The exact parameters and filters available for a given endpoint are listed in the [API reference](/api).
...
Refer to the [API reference](/api) for the exact request body schema of each endpoint.
