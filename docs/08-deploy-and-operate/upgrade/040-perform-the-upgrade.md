...
nav_order: 40
permalink: /operate/upgrade/perform-the-upgrade
content_type: how-to
redirect_from: ["/paas-operations/upgrade/guide/perform-the-upgrade"]
source_path: docs/190-paas-operations/upgrade/upgrade-guide/030-perform-for-the-upgrade.md
...
This page covers stage 3 of the [CluedIn upgrade process](/operate/upgrade/guide). It provides step-by-step instructions to apply the new version of CluedIn safely and verify that the upgrade completes successfully.

{:.important}
Before you begin, make sure that you have completed all actions outlined in [Plan the upgrade](/operate/upgrade/plan-the-upgrade) and [Prepare for the upgrade](/operate/upgrade/prepare-for-the-upgrade).
...
The [Helm](/operate/upgrade/required-tools#helm) user values file is a YAML file that defines the configuration values to be applied when upgrading a [Kubernetes](/operate/upgrade/required-tools#kubernetes) cluster. The file will look similar to the following:
...
1. YAML file is created in your working directory. Open the file in your preferred IDE (we typically use [Visual Studio Code](/operate/upgrade/required-tools#visual-studio-code)). 
...
1. Make sure that your packages are ready to be installed in the new version. For details, see [Prepare and test custom packages](/operate/upgrade/plan-the-upgrade#prepare-and-test-custom-packages).

1. The required values for the target release are published in [CluedIn documentation](/operate/upgrade). Before proceeding, carefully verify that your current release is compatible with the target release. Once verified, duplicate the values file created in the previous step and rename it to match the target release. 
...
1. Carefully compare your existing values file with the new release’s required values. Update any outdated entries and add newly introduced values:
    ...
    - An IDE such as [Visual Studio Code](/operate/upgrade/required-tools#visual-studio-code) will highlight any formatting or indentation issues as you edit the YAML file.
...
1. Complete the following checks:
    ...
    - [Verify](/operate/upgrade/common-upgrade-operations#check-cluedin-ui) that CluedIn UI is running correctly.

    - [Confirm](/operate/upgrade/common-upgrade-operations#check-cluedin-pods) that all pods are in a healthy (green) state.

    - [Review](/operate/upgrade/common-upgrade-operations#check-cluedin-logs) the server logs and ensure they are free of errors.
...
1. [Check](/operate/upgrade/common-upgrade-operations#check-cluedin-queues) internal CluedIn queues.

## Perform Helm upgrade

There are several methods to upgrade [Helm](/operate/upgrade/required-tools#helm):
...
During a [Helm](/operate/upgrade/required-tools#helm) upgrade, the UI will be temporarily unavailable. Make sure to notify all users in advance so that they are aware of the downtime. 
...
1. To monitor the upgrade progress, check the following: 
    ...
    - [CluedIn pods](/operate/upgrade/common-upgrade-operations#check-cluedin-pods)
    - [CluedIn server logs](/operate/upgrade/common-upgrade-operations#check-cluedin-logs)
...
1. Check [CluedIn UI](/operate/upgrade/common-upgrade-operations#check-cluedin-ui) and ensure everything is running smoothly.
