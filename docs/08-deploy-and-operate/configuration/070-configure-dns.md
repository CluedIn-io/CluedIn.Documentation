...
nav_order: 70
permalink: /operate/configuration/configure-dns
content_type: how-to
redirect_from: ["/deployment/infra-how-tos/configure-dns"]
source_path: docs/190-paas-operations/configuration/080-configure-dns.md
...
In this article, we'll use `cluedin` as the example organization name that was entered during the [installation of CluedIn](/operate/azure-marketplace/step-3#complete-the-instance-setup-tab) and `yourdomain.com` as the example main domain.
...
- You should be connected to your AKS cluster.
    ...
    See [Connect to CluedIn cluster](/operate/configuration/connect-to-cluedin) for detailed instructions.
...
{:.important}
This will use the LetsEncrypt service in the cluster to do an HTTP request to validate the certificate. If you would like to use a self-provided certificate, please review the [Configure TLS Certificates](/operate/configuration/configure-certificates) page.