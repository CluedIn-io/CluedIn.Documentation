...
grand_parent: Master
nav_order: 10
permalink: /master/golden-records/golden-record-relations
content_type: how-to
redirect_from: ["/key-terms-and-features/golden-records/golden-record-relations"]
source_path: docs/110-key-terms-and-features/golden-records/010-golden-record-relations.md
...
Relations connect golden records by linking a source record to a target record through a specific edge type (for example, _/WorksFor_ or _/FinancedBy_). These relations can be created either before or after data is [processed](/ingest/processing/process-data).
...
- When ingesting data into CluedIn. This method is useful when adding relations for multiple records at once. For details, see [Add relations between records](/model/relationships/tutorial-add-relations).
...
1. Configure **Graph Display Settings**:
    ...
    - **Include Shadow Records** – when this option is enabled, the graph will also show all [shadow records](/get-started/core-concepts/eventual-connectivity) associated with the golden records in it. This option may be useful when debugging relations between golden records.
        ...
        {:.important}
        Shadow records are records that exist in CluedIn but are flagged as hidden. This typically happens when a record does not have enough values to be promoted to a golden record and contains only [identifiers](/model/identifiers).
   ...
   - **Include Enricher Data** – when this option is enabled, the graph will also show records that consist only of data provided by [enrichers](/govern/enrichment).
