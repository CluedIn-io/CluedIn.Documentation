...
$yaml = Get-Content "$($cluedinInstallFolder)$($sep)values.yml" | ConvertFrom-Yaml
...
$choice = Read-Host
if((-not [System.String]::IsNullOrWhiteSpace($choice)) -and ($choice.ToLower().Equals('y'))){
	Write-Host "Please enter the custom prefix for the App URL, <appPrefix>.$hostname (Only alphanumeric and '-' are allowed): " -NoNewLine
	$appPrefix = Read-Host
	$yaml.global.dns.subdomains.application = $appPrefix
	
	Write-Host "Please enter the custom prefix for the Clean URL, <cleanPrefix>.$hostname (Only alphanumeric and '-' are allowed): " -NoNewLine
	$cleanPrefix = Read-Host
	$yaml.global.dns.subdomains.openrefine = $cleanPrefix
	
	Write-Host "Please enter the custom prefix for the Grafana URL, <grafanaPrefix>.$hostname (Only alphanumeric and '-' are allowed): " -NoNewLine
	$grafanaPrefix = Read-Host
	$yaml.global.dns.subdomains.grafanaAdmin = $grafanaPrefix
	
	Write-Host "Please enter the custom prefix for the Promotheus URL, <promotheusPrefix>.$hostname  (Only alphanumeric and '-' are allowed): " -NoNewLine
	$promotheusPrefix = Read-Host
	$yaml.global.dns.subdomains.prometheusAdmin = $promotheusPrefix
	
	Write-Host "Please enter the custom prefix for the Alert Manager URL, <alertManagerPrefix>.$hostname  (Only alphanumeric and '-' are allowed): " -NoNewLine
	$alertMgrPrefix = Read-Host
	$yaml.global.dns.subdomains.alertManagerAdmin = $alertMgrPrefix
	
	Write-Host "Please enter the custom prefix for the Seq URL, <seqPrefix>.$hostname  (Only alphanumeric and '-' are allowed): " -NoNewLine
	$seqPrefix = Read-Host
...
Write-Host "Do you wish to configure SSL/HTTPS for your CluedIn instance? (Y/N): " -NoNewLine
$choice = Read-Host
if((-not [System.String]::IsNullOrWhiteSpace($choice)) -and ($choice.ToLower().Equals('y'))){
	$yaml.global.ingress.forceHttps = true
	Write-Host " Configuring hosts (from your SSL certificate): " -ForegroundColor Yellow
	Write-Host "  Does your certificate have:"
	Write-Host "   1- Wildcard host *.$hostname"
	Write-Host "   2- Specific named hosts: $appPrefix.$hostname, $cleanPrefix.$hostname, $orgPrefix.$hostname etc..."
	...
	if($choice.Equals("1")){
		...
		foreach($sd in $secondarySubdomains){
			Write-Host "  Add $sd to hosts? (Y/N)" -NoNewLine
			$decision = Read-Host
	...
	Write-Host "  Enter the name of the secret (only letters and hyphens), or press Enter to use the default (ssl-cert-secret): " -NoNewLine
	$sslSecretName = Read-Host
	if([System.String]::IsNullOrWhiteSpace($sslSecretName)){
		$sslSecretName = "ssl-cert-secret"
	}
	Write-Host "  Enter the path to the tls.key file (private key): " -NoNewLine
	...
	Write-Host "  Enter the path to the tls.crt file (public certificate): " -NoNewLine
...
($yaml | ConvertTo-Yaml) | Out-File -FilePath "$($cluedinInstallFolder)$($sep)values.yml"
